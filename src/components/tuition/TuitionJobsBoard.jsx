  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: tuitions, error: tuitionsError } = await supabase
        ...
        `)
        .eq('status', 'open')
        .eq('has_custom_institution', false)
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex justify-end">
        ...
        >
          {showCityFilter ? 'Hide City Filter' : 'Show All Cities'}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {paginatedTuitions.map(job => {
            ...
            return (
              <div 
                key={job.id} 
                className="bg-white rounded-3xl p-6 border border-slate-100 border-l-4 border-l-[#86c240] shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(134,194,64,0.06)] hover:border-l-[#6a9c31] transition-all duration-300 flex flex-col justify-between h-full group animate-in fade-in duration-300"
              >
                ...
                <div className="flex justify-between items-start gap-3 mb-4">
                  <div className="space-y-1">
                    ...
                    {job.has_custom_institution && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-amber-50 border border-amber-200 text-amber-600 font-extrabold rounded-full text-[9px] tracking-wide animate-pulse">
                        ⚠️ Pending Review
                      </span>
                    )}
                  </div>
                ...
                {/* Location and Date */}
                <div className="flex items-center gap-2 text-xs font-bold text-slate-700 bg-slate-50 rounded-2xl px-3 py-2.5 mb-5">
                  <MapPin className="w-3.5 h-3.5 text-[#86c240] flex-shrink-0" />
                  <span className="truncate">{job.location}</span>
                  <span className="text-slate-300 ml-auto flex-shrink-0 font-medium">{postDate}</span>
                </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 max-h-16 overflow-y-auto pr-1">
                    {job.subject && job.subject.length > 0 ? (
                      job.subject.map((sub, idx) => (
                        <span 
                          key={idx} 
                          className="text-[10px] font-extrabold bg-slate-50 text-slate-600 px-2 py-0.5 rounded-md border border-slate-100"
                        >
                          {sub}
                        </span>
                      ))
                    ) : (
                      <span className="text-[10px] font-extrabold bg-slate-50 text-slate-400 px-2 py-0.5 rounded-md">N/A</span>
                    )}
                  </div>
                </div>

                {/* Key Details Grid */}
                <div className="grid grid-cols-2 gap-4 border-t border-b border-slate-100/80 py-4 mb-5">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-xl bg-[#86c240]/10 text-[#86c240] flex items-center justify-center flex-shrink-0">
                ...
                {profile?.role === 'tutor' && !reqCheck.match && (
                  <div className="bg-red-50 border border-red-100 rounded-2xl p-3 text-[10px] text-red-650 font-bold mb-4 flex items-start gap-1.5">
                    <span className="flex-shrink-0">⚠️</span>
                    <span>{reqCheck.reason}</span>
                  </div>
                )}
                ...
                {profile?.role !== 'guardian' && (
                  <div className="flex justify-between items-center mt-auto pt-1">
                    <Link 
                      to={`/tuition/${job.id}`} 
                      className="text-xs font-bold text-slate-500 hover:text-[#86c240] transition-colors"
                    >
                      View Details
                    </Link>
                    <button 
                      ...
                      className={`text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-sm ${
                        profile?.role === 'tutor' && !reqCheck.match
                          ...
                          : 'bg-[#86c240] hover:bg-[#6a9c31] text-white hover:shadow-md'
                      }`}
